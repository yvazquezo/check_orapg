...
BEGIN
	...
	RETURN NEXT ('Tablespaces'::text, v_total_tbspc);
	SELECT count(*) INTO v_total_users FROM mig_users WHERE username NOT IN ('ORDPLUGINS','MGMT_VIEW','MDSYS','ORDSYS','SYSMAN','DBSNMP','OWBSYS_AUDIT','SI_INFORMTN_SCHEMA','APEX_030200','ANONYMOUS','APEX_PUBLIC_USER','APPQOSSYS','CTXSYS','DIP','EXFSYS','FLOWS_FILES','MDDATA','OLAPSYS','ORACLE_OCM','ORDDATA','OUTLN','OWBSYS','SCOTT','SPATIAL_CSW_ADMIN_USR','SPATIAL_WFS_ADMIN_USR','SYS','SYSTEM','WMSYS','XDB','XS$NULL');
	RETURN NEXT ('Users'::text, v_total_users);
	SELECT count(*) INTO v_total_roles FROM mig_roles WHERE role NOT IN ('ADM_PARALLEL_EXECUTE_TASK','APEX_ADMINISTRATOR_ROLE','AQ_USER_ROLE',
		...
		'XDB_WEBSERVICES','XDB_WEBSERVICES_OVER_HTTP','XDB_WEBSERVICES_WITH_PUBLIC','DELETE_CATALOG_ROLE','AQ_ADMINISTRATOR_ROLE','CWM_USER',
		'OLAP_DBA','OLAP_USER','OLAP_XS_ADMIN','OWB$CLIENT','OWB_DESIGNCENTER_VIEW','OWB_USER','MGMT_USER');
	RETURN NEXT ('Roles'::text, v_total_roles);
	...
	SELECT COUNT(*) INTO v_total_synonyms FROM mig_SYNONYMS JOIN mig_OBJECTS on (mig_SYNONYMS.table_owner=mig_OBJECTS.owner and mig_SYNONYMS.table_name=mig_OBJECTS.object_name) WHERE TABLE_OWNER IN (upper($1),'PUBLIC') AND STATUS <> 'INVALID';
	RETURN NEXT ('Synonyms'::text, v_total_synonyms);  
...
BEGIN
	...
	v_query := '
	with login_roles as (
	select pr.username from mig_users pr where pr.username not in (''ORDPLUGINS'',''MGMT_VIEW'',''MDSYS'',''ORDSYS'',''SYSMAN'',''DBSNMP'',''OWBSYS_AUDIT'',''SI_INFORMTN_SCHEMA'',''APEX_030200'',''ANONYMOUS'',
		...
		''SYS'',''SYSTEM'',''WMSYS'',''XDB'',''XS$NULL'',''PERFSTAT'',''PSM''))
...
BEGIN
	...
	select pr.role from mig_roles pr where role not in (''ADM_PARALLEL_EXECUTE_TASK'',''APEX_ADMINISTRATOR_ROLE'',
		...
		''DBFS_ROLE'',''ORDADMIN'',''RECOVERY_CATALOG_OWNER'',''RESOURCE'',''SCHEDULER_ADMIN'',''SELECT_CATALOG_ROLE'',''SPATIAL_CSW_ADMIN'',
		''SPATIAL_WFS_ADMIN'',''WFS_USR_ROLE'',''WM_ADMIN_ROLE'',''XDBADMIN'',''XDB_SET_INVOKER'',''XDB_WEBSERVICES'',''XDB_WEBSERVICES_OVER_HTTP'',
		''XDB_WEBSERVICES_WITH_PUBLIC'',''DELETE_CATALOG_ROLE'',''AQ_ADMINISTRATOR_ROLE'',''CWM_USER'',''OLAP_DBA'',''OLAP_USER'',''OLAP_XS_ADMIN'',''OWB$CLIENT'',''OWB_DESIGNCENTER_VIEW'',
		''OWB_USER'',''MGMT_USER''))
	select rl.role::text, count(*) filter (where grantee is not null)
	from login_roles rl left join
...
BEGIN
	...
	(
		...
		AND object_name NOT LIKE ''QUEST_SL_TEMP_EXPLAIN%'' AND object_name NOT LIKE ''SYS_NT0s2%'' and object_name not ilike ''dr$%'' and object_name not like ''TOAD_PLAN_TABLE''
		AND temporary <> ''Y''
		GROUP BY OWNER),
	total_views as
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''VIEW'' AND STATUS <> ''INVALID''
		GROUP BY OWNER),
	total_mviews as
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''MATERIALIZED VIEW'' AND STATUS <> ''INVALID''
		GROUP BY OWNER),
	total_triggers as
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''TRIGGER'' AND STATUS <> ''INVALID''
		GROUP BY OWNER
		ORDER BY 1),
	total_synonyms as
	(
		SELECT TABLE_OWNER AS s_schema, COUNT(*) AS s_synonyms
		FROM mig_SYNONYMS JOIN mig_OBJECTS on (mig_SYNONYMS.table_owner=mig_OBJECTS.owner and mig_SYNONYMS.table_name=mig_OBJECTS.object_name)
		WHERE TABLE_OWNER IN ('||$1||') AND STATUS <> ''INVALID''
	...
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''SEQUENCE'' AND STATUS <> ''INVALID''
		GROUP BY OWNER
		ORDER BY 1),
	total_indexes as
	(
		...
		and index_type <> ''LOB'' AND o.STATUS <> ''INVALID''
		GROUP BY o.OWNER),
	total_packages as
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''PACKAGE'' AND STATUS <> ''INVALID''
		GROUP BY OWNER),
	total_bodies as
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''PACKAGE BODY'' AND STATUS <> ''INVALID''
		GROUP BY OWNER
		ORDER BY 1),
	total_func as
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''FUNCTION'' AND STATUS <> ''INVALID''
		GROUP BY OWNER),
	total_proc as
	(
		...
		FROM mig_OBJECTS
		WHERE OWNER IN ('||$1||')
		AND object_type LIKE ''PROCEDURE'' AND STATUS <> ''INVALID''
		GROUP BY OWNER),
	total_cons as
	(
...
BEGIN
	...
	(
		SELECT TABLE_OWNER AS s_schema, COUNT(*) AS s_synonyms
		FROM mig_SYNONYMS JOIN mig_OBJECTS on (mig_SYNONYMS.table_owner=mig_OBJECTS.owner and mig_SYNONYMS.table_name=mig_OBJECTS.object_name)
		WHERE TABLE_OWNER NOT IN (''SYS'',''SYSTEM'',''XDB'',''APPQOSSYS'',''DBSNMP'',''FLOWS_FILES'',''OLAPSYS'',''ORDDATA'',''OUTLN'',''OWBSYS'',''WMSYS'') AND STATUS <> ''INVALID''
		GROUP BY TABLE_OWNER),
	total_sequences as
	(
		SELECT OWNER AS s_schema, COUNT(*) AS s_sequences
...
BEGIN
	...
	-- Attributes by tables
	FOR v_query IN EXECUTE 'select lower(c.owner) as s_schema, lower(table_name) as s_table, count(*) as s_total
			from mig_tab_columns c join mig_objects o on (c.owner=o.owner and c.table_name=o.object_name)
			where c.owner in ('||upper($1)||') and table_name not ilike ''dr$%'' and table_name not ilike ''bin$%'' and table_name not ilike ''toad_plan_table''
				and table_name not in (select table_name from mig_tables where temporary = ''Y'') and o.status = ''VALID''
			group by lower(c.owner), lower(table_name)'
	LOOP
	...
	-- Constraints by schema
	FOR v_query IN EXECUTE 'select lower(owner) as s_schema, count(*) filter (where constraint_type=''P'') as s_pk, count(*) filter (where constraint_type=''R'') as s_fk, count(*) filter (where constraint_type=''C'' and c.search_condition not ilike ''%" is not null%'') as s_check, count(*) filter (where constraint_type=''U'') as s_unique
			from (select distinct c.owner, c.constraint_name, c.constraint_type, c.table_name, c.search_condition
			from mig_constraints c join mig_cons_columns cc on (c.owner=cc.owner and c.constraint_name=cc.constraint_name)
...
BEGIN
	...
	ELSE
		...
		LOOP
			RETURN NEXT (' ' || rpad(v_res.schema_ref, schema_size+1, ' ') || '| ' || rpad(v_res.name_ref, object_size+1, ' ') || '|' || lpad(v_res.description::text, total_size+1, ' '));
		END LOOP;
		RETURN NEXT '';
	END IF;
	RETURN NEXT '';
